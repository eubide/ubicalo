...
- **El Simulacro guarda lo escrito por Alcance y el mapa que el alumno tiene delante.** Político no tiene un Todo que reutilizar y sus dos mapas se solapan, así que lleva un conmutador sobre una sola cuenta atrás; al volver de un corte se vuelve al mapa en el que se estaba. La Nota es una sobre los 71, el aviso de blancos dice cuántos quedan en cada mapa, y el mapa corregido se mira con el mismo conmutador. Un Simulacro ya entregado no se vuelve a guardar al cambiar de mapa: reaparecería como en curso y se anotaría dos veces.
- Quedan fuera hasta ver a alumnos usar esto: el Reto de Simulacro, reinsertar los pares que se confunden, quitar Elementos de la Familia con «Esto no me entra», Localizar dentro de las Tandas, la fecha de examen y medir el uso. El mapa mudo imprimible estaba en esta lista y salió de ella en el ADR-0013, porque la señal que la desbloquea no es observable: sin analítica y sin canal con los alumnos, «ver a alumnos usar esto» no llega nunca.
